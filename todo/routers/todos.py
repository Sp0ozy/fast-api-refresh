...
async def render_todo_page(request: Request, db: db_dependency):
    try:
        ...
        print(f"[todo-page error] {type(e).__name__}: {e}")
        return redirect_to_login()
...
async def render_edit_todo_page(request: Request, todo_id: int, db: db_dependency):
    try:
        ...
        return templates.TemplateResponse(request, "edit-todo.html", {"user": user, "todo": todo})
    except Exception:
        return redirect_to_login()



# API routes used by base.js (send Authorization: Bearer header)
@router.post("/todo", status_code=status.HTTP_201_CREATED)
async def create_todo_form(user: user_dependency, db: db_dependency, todo_request: TodoRequest):
    todo_model = Todos(**todo_request.model_dump(), owner_id=user.get("id"))
    db.add(todo_model)
    db.commit()

@router.put("/todo/{todo_id}", status_code=status.HTTP_204_NO_CONTENT)
async def update_todo_form(user: user_dependency, db: db_dependency, todo_request: TodoRequest, todo_id: int = Path(gt=0)):
...
async def delete_todo_form(user: user_dependency, db: db_dependency, todo_id: int = Path(gt=0)):
    todo_model = db.query(Todos).filter(Todos.id == todo_id)\
        .filter(Todos.owner_id == user.get("id")).first()
    if todo_model is None:
        raise HTTPException(status_code=404, detail="Todo not found")
